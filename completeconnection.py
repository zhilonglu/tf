...
def fcn(trainX,trainY,hiddennum,times,keep,modelname,cutime,validX=None,validY=None,valid=False):
    ...
    for i in range(len(nodenums)-1):
        ...
        bs.append(bi)
        if i<len(nodenums)-2:
            hiddeni = tf.nn.elu(tf.add(tf.matmul(drops[i],Wi),bi))
...
pred([24,12,12],6,taskname, cutime, preX)
 
# for i in range(10):
#     cutime=datetime.strftime(datetime.now(),"%Y%m%d%H%M%S")
#     print(cutime)
#     path="E:\\kddcup\\tensordir\\"+taskname+"\\"+cutime+"\\"
#     os.makedirs(path)
#     print(path)
#     lossi=fcn(trainX, trainY, [24,12,12], int(1.5e4), 1, taskname, cutime, validX, validY, True)
